    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                debug_image,x,y = draw_landmarks(debug_image, landmark_list,x,y)
...
def pre_process_landmark(landmark_list):
    k = np.array(landmark_list)
    temp_landmark_list = copy.deepcopy(k[:,:2])
...
def draw_landmarks(image, landmark_point_ori,x,y):
    
    landmark_point = np.array(landmark_point_ori)[:,:2]
    ...
    text_x = image.shape[1] - text_size[0] - 10  # 10 pixels from right
    text_y = text_size[1] + 10  # 10 pixels from top
    
    cv.putText(image, text, (text_x, text_y), font, font_scale, (0,0,255), 3)
    for landmark in landmark_point_ori:
